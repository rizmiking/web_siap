...
<div x-data="{ 
    ...
    <div class="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500">Kelola berkas dan persyaratan administrasi secara dinamis untuk kegiatan ini[cite: 1].</p>
        </div>
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">
                    Kelola seluruh dokumen dan persyaratan administrasi pelatihan[cite: 1].
                </p>
            </div>
        ...
        @if($administrasis->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                ...
                @foreach ($administrasis as $item)
                    ...
                    <div class="group bg-white rounded-2xl border border-slate-200 hover:border-blue-200 hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col justify-between">
                        ...
                        <div class="p-5">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div>
                                    @if($item->wajib)
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-rose-50 text-rose-600 text-[10px] font-bold uppercase">
                                            <span class="w-1.5 h-1.5 rounded-full bg-rose-500"></span>
                                            Wajib
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold uppercase">
                            ...
                            <div class="mt-4">
                                ...
                                <div class="mt-2">
                                    <span class="inline-flex px-2 py-1 rounded-md bg-slate-100 text-slate-600 text-[10px] font-medium">
                                        {{ $item->jenis }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <!-- Status -->
                            <div class="mt-5 p-3 rounded-xl {{ $item->status == 'Sudah Diunggah' ? 'bg-emerald-50 border border-emerald-100' : ($item->status == 'Diverifikasi' ? 'bg-blue-50 border border-blue-100' : 'bg-amber-50 border border-amber-100') }}">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs font-bold {{ $item->status == 'Sudah Diunggah' ? 'text-emerald-700' : ($item->status == 'Diverifikasi' ? 'text-blue-700' : 'text-amber-700') }}">
                                                {{ $item->status }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    @if($item->file)
                                        <a href="{{ asset('storage/' . $item->file) }}" target="_blank" class="text-xs font-semibold text-blue-600 hover:text-blue-700 hover:underline">
                                            Lihat File &rarr;
                                        </a>
                                    @endif
                                </div>
                            ...
                            <div class="mt-4 min-h-[42px]">
                                <p class="text-[10px] uppercase font-semibold text-slate-400 mb-1">
                                    Keterangan
                                </p>
                        ...
                        <div class="px-5 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                            <button @click="openEdit({{ json_encode($item) }})" class="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-600 hover:text-amber-700 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                {{ Auth::user()->isSuperAdmin() ? 'Edit / Upload' : 'Upload File' }}
                            </button>
                            ...
                            @if (Auth::user()->isSuperAdmin())
                                <form action="{{ route('administrasi.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus dokumen administrasi ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center gap-1.5 text-xs font-semibold text-red-500 hover:text-red-700 transition">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @else
            ...
            <div class="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center">
                <div class="mx-auto w-16 h-16 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-500 mb-4">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414A1 1 0 0119 9v10a2 2 0 01-2 2z" />
                    </svg>
                </div>
                ...
                <p class="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                    Belum ada dokumen administrasi yang ditentukan untuk pelatihan ini.
                </p>

                @if (Auth::user()->isSuperAdmin())
                    <button @click="showCreateModal = true" class="mt-5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold transition">
                        + Tambah Dokumen
                    </button>
                @endif
            </div>
    ...
    <!-- ==================== MODAL TAMBAH ADMINISTRASI (SUPERADMIN ONLY) ==================== -->
    @if (Auth::user()->isSuperAdmin())
        ...
        <div @click.away="showEditModal = false" class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-slate-100">
            <div class="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
                <h3 class="text-lg font-bold text-slate-800">
                    {{ Auth::user()->isSuperAdmin() ? 'Edit / Unggah Berkas Administrasi' : 'Upload Berkas Administrasi' }}
                </h3>
            ...
            <form :action="'/administrasi/' + editData.id" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                @else
                    <!-- Admin Viewer Preview Informasi Dokumen -->
                    <div class="p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs space-y-1">
                        <p><span class="font-semibold text-slate-700">Nama Dokumen:</span> <span x-text="editData.nama"></span></p>
                        <p><span class="font-semibold text-slate-700">Jenis:</span> <span x-text="editData.jenis"></span></p>
                        <p><span class="font-semibold text-slate-700">Sifat:</span> <span x-text="editData.wajib == 1 ? 'Wajib' : 'Opsional'"></span></p>
                    </div>
                @endif
